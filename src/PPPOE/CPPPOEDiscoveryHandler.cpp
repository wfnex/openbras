...
void CPPPOEDiscoveryHandler::processPADR(PPPoEPacket *packet, SWORD32 len)
{
    ...
    {
	    ACE_DEBUG((LM_ERROR, 
                   "CPPPOEDiscoveryHandler::processPADR(), the server has reached maximum sessions, no valid session id\n"));
	    sendErrorPADS(packet->ethHdr.h_source, 
        			  TAG_SERVICE_NAME_ERROR, 
        			  "WFNEX-PPPoE: Server: maximum sessions reached, no valid session id");
	    return;        
    }
